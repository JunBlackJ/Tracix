...
async function generateAlerts(orgId) {
    ...
    const departedActiveMembers = await client_1.default.member.findMany({
        where: {
    ...
    const overdueMembers = await client_1.default.member.findMany({
        where: { organization_id: orgId },
        include: {
            accessRights: true,
        },
    ...
    const sharedAccountMembers = await client_1.default.member.findMany({
        where: {
            organization_id: orgId,
    ...
    if (newAlerts.length > 0) {
        await client_1.default.alert.createMany({ data: newAlerts });
    }
}
