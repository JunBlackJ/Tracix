...
router.post('/:id/entries', async (req: Request, res: Response): Promise<void> => {
  ...
  const entry = await prisma.customEntry.create({
    data: { id: uuidv4(), module_id: req.params.id, data: body.data as import('@prisma/client').Prisma.InputJsonValue },
  });
...
router.put('/:id/entries/:entryId', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!mod) { res.status(404).json({ error: 'Module non trouvé' }); return; }
  const body = EntrySchema.parse(req.body);
  const updated = await prisma.customEntry.update({ where: { id: req.params.entryId }, data: { data: body.data as import('@prisma/client').Prisma.InputJsonValue } });
  res.json(updated);
});
